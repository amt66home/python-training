...
# that the range function returns a new list with numbers of that specified 
# range, whereas xrange returns an iterator, which is more efficient. 
# (Python 3 uses the range function, which acts like xrange). Note that 
...
# the "for" or "while" statement. A few examples:
count = 0
while True:
    print(count)
    count += 1
...
# code part in "else" is executed. If a break statement is 
# executed inside the for loop then the "else" part is skipped. 
# Note that the "else" part is executed even if there is a continue 
# statement.
...
while(count<5):
    ...
    print("count vlaue reached %d" %(count))
...
for i in range(1, 10):
    ...
    print("This is not printed because for loop is terminated because of break but not due to fail in conditon")
